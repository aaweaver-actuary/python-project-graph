...
`completion-ledger.md` is the authoritative live completion artifact for the active request.

A materially new user request that changes completion semantics, delivery shape, or PR strategy must start from a fresh active ledger state or an archived prior ledger. Previously completed requests must not remain the active completion artifact when they would bias the current run.

## Rules

- Project Manager is the only role allowed to finalize `completion-ledger.md` state transitions.
- The active ledger must describe only the current request’s completion model.
- If a prior request was completed under a different delivery contract, archive or replace that ledger state before continuing.
- Do not treat prior `complete` status as transferable when the new request changes issue scope, acceptance semantics, bundling strategy, or PR strategy.
- A rebaselined request must explicitly record that prior completion state is no longer the active source of truth.
- If there is any doubt whether the request is materially new, default to starting from a fresh active ledger state.
...
- issue set changes enough that prior completion evidence no longer maps cleanly
- acceptance criteria change in a way that invalidates prior completion status
- the requested delivery artifact changes materially
- the user explicitly reframes what “done” means

## Required Project Manager Behavior

When a materially new request is detected, Project Manager must:

1. stop treating the prior active ledger as the source of truth for completion of the new request
2. create or initialize a fresh active ledger state for the new request, or explicitly archive the prior one
3. restate the new request scope and completion semantics in the active ledger
4. continue orchestration only from the fresh active ledger state

## Prohibited Behavior

- Do not continue using an old active ledger that already says `complete` when the new request changes completion semantics.
- Do not infer that previously satisfied slices imply completion of the current request unless they are explicitly revalidated against the new ledger.
- Do not let administrative continuity override current completion truth.
