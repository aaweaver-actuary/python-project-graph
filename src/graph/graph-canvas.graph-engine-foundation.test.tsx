...
const { reactFlowPropsSpy } = vi.hoisted(() => ({
...
describe("GraphCanvas graph engine foundation (WU-01)", () => {
  ...
  it("renders a React Flow root while preserving node and edge identity hooks", () => {
    ...
    const renderedNodes = within(reactFlowRoot).getAllByTestId(GRAPH_NODE_TEST_ID);
    const renderedEdges = within(reactFlowRoot).getAllByTestId(GRAPH_EDGE_TEST_ID);
